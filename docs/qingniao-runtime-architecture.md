# Qingniao delegated-execution runtime architecture (pre-extraction)

> **Migration note:** this document preserves the detailed delegated-execution
> runtime design implemented under the original `Marang` namespace. Under
> [ADR 0015](decisions/0015-qingniao-extraction-and-marang-service-boundary.md),
> every delegated-execution responsibility and contract described below migrates to
> `Penghou.Qingniao`; Marang retains only the ASP.NET Core MVC/MCP service
> boundary. Names remain unchanged here only until the source refactor makes the
> ownership mechanically true.
>
