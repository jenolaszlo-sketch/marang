# Qingniao delegated-execution runtime roadmap (pre-extraction history)

> **Migration note:** Batches 1–7 and the completed Milestone 2 sub-batches
> were implemented under the original `Marang` project names. They are now the
> seed of `Penghou.Qingniao.Abstractions` and `Penghou.Qingniao`. This file
> preserves detailed implementation progress; the active Marang service plan is
> [roadmap.md](roadmap.md). See
